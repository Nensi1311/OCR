...
file_path = "pdf/9784 Narration Issue (1).pdf"
...
for line in TOTAL_TEXT:
    ...
    if raw_date and str(raw_date).replace('/', '').replace('-', '').replace('\n', '').strip().isdigit():
       
        formatted_date = str(raw_date).replace('/', '-').replace('\n', '').strip()
        narration = line[3].replace('\n', ' ') if (len(line) > 3 and line[3]) else ""
        
        debit_val = line[5] if (len(line) > 5 and line[5]) else "0.00"
        credit_val = line[6] if (len(line) > 6 and line[6]) else "0.00"
        
        cheque_num = line[4].strip() if (len(line) > 4 and line[4]) else ""
